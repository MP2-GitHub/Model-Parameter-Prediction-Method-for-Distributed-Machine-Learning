...
import utils
import ResNet34_functions
import numpy as np

FLAGS = tf.app.flags.FLAGS
# Neural Network Configuration
tf.app.flags.DEFINE_integer('nums_worker', 1, """Number of workers.""")
tf.app.flags.DEFINE_integer('parameters_length', 22670052, """Number of model parameters byte.""")
...
class BSP():
    ...
    def predict(self):
        x = ResNet34_functions.create_placeholder()
        parameters_tensor = ResNet34_functions.inference(x)
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, './save_model_7438/model.ckpt')
            while True:
                paras = np.zeros([1, 351872])
                ...
                paras = paras.reshape([-1, 8, 8, 5498])
...
def main():
    ...
    server_socket = ResNet34_functions.tcp_connection(FLAGS.port, FLAGS.nums_worker)
    queue = ResNet34_functions.create_queue(FLAGS.nums_worker)
    sockets = []
    worker_id = 1
    while True:
        ...
        if len(sockets) == FLAGS.nums_worker:
            ...
            for socket in sockets:
                bsp_init = threading.Thread(target=ResNet34_functions.send_init_parameters, args=(socket, init_parameters, worker_id))
                bsp_init.start()
                bsp_init.join()
                worker_id += 1
            worker_id = 1
