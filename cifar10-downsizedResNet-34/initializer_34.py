...
def initial_parameters():
    ...
    L = len(initial_parameter) // 2
    shape = {}
    for i in range(L):
        shape['dw' + str(i + 1)] = [initial_parameter['w' + str(i + 1)].shape,
                                   tuple_multi(initial_parameter['w' + str(i + 1)].shape)]
        shape['db' + str(i + 1)] = initial_parameter['b' + str(i + 1)].shape

    print(shape)

    return initial_parameter, shape

def tuple_multi(ytuple):
