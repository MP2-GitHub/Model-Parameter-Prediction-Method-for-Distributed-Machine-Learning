...
def create_placeholder():
    ...
    # weights samples
    x = tf.placeholder(tf.float32, [None, 8, 8, 5498], name='input_x')  # 617344/64
...
def forward_propagation(input_tensor, input_images, seed):
    ...
    with tf.variable_scope('Decoder-2'):
        w37 = tf.get_variable('fcn1-weight', [flatten.shape[1], 2570], initializer=tf.truncated_normal_initializer(stddev=0.1))
        z37 = tf.matmul(flatten, w37)
        y37_hat = tf.reshape(z37, [-1, 2570])
    ...
    decoder_fcn_w37 = y37_hat[seed, 0: 2560]
    decoder_fcn_w37 = tf.reshape(decoder_fcn_w37, [256, 10])
    decoder_fcn_b37 = y37_hat[seed, 2560: 2570]
