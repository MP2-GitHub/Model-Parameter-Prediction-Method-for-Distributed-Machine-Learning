...
def initialize_parameters():
    ...
    w11 = np.random.randn(CONV11_SIZE, CONV11_SIZE, CONV10_DEEP, CONV11_DEEP) / np.sqrt(CONV11_SIZE * CONV11_SIZE * CONV10_DEEP)
    w12 = np.random.randn(CONV12_SIZE, CONV12_SIZE, CONV11_DEEP, CONV12_DEEP) / np.sqrt(CONV12_SIZE * CONV12_SIZE * CONV11_DEEP)
    w13 = np.random.randn(CONV13_SIZE, CONV13_SIZE, CONV12_DEEP, CONV13_DEEP) / np.sqrt(CONV13_SIZE * CONV13_SIZE * CONV12_DEEP)

    w14 = np.random.randn(256, FC1_SIZE) / np.sqrt(256)
    w15 = np.random.randn(FC1_SIZE, FC2_SIZE) / np.sqrt(FC1_SIZE)
    w16 = np.random.randn(FC2_SIZE, NUM_LABELS) / np.sqrt(FC2_SIZE)
    ...
    b11 = np.zeros((CONV11_DEEP,))
    b12 = np.zeros((CONV12_DEEP,))
    b13 = np.zeros((CONV13_DEEP,))

    b14 = np.zeros((FC1_SIZE,))
    b15 = np.zeros((FC2_SIZE,))
    b16 = np.zeros((NUM_LABELS,))

    init_parameters = {"w1": w1, "w2": w2, "w3": w3, "w4": w4, "w5": w5, "w6": w6, "w7": w7, "w8": w8, "w9": w9,
                       "w10": w11, "w11": w12, "w12": w13, "w13": w14, "w14": w15, "w15": w16,
                       "b1": b1, "b2": b2, "b3": b3, "b4": b4, "b5": b5, "b6": b6, "b7": b7, "b8": b8, "b9": b9,
                       "b10": b11, "b11": b12, "b12": b13, "b13": b14, "b14": b15, "b15": b16
                       }
...
def create_placeholder():
    ...
    cv11_w = tf.placeholder(tf.float32, [CONV11_SIZE, CONV11_SIZE, CONV10_DEEP, CONV11_DEEP], name='cv11_w')
    cv11_b = tf.placeholder(tf.float32, [CONV11_DEEP], name='cv11_b')

    cv12_w = tf.placeholder(tf.float32, [CONV12_SIZE, CONV12_SIZE, CONV11_DEEP, CONV12_DEEP], name='cv12_w')
    cv12_b = tf.placeholder(tf.float32, [CONV12_DEEP], name='cv12_b')

    cv13_w = tf.placeholder(tf.float32, [CONV13_SIZE, CONV13_SIZE, CONV12_DEEP, CONV13_DEEP], name='cv13_w')
    cv13_b = tf.placeholder(tf.float32, [CONV13_DEEP], name='cv13_b')

    fc14_w = tf.placeholder(tf.float32, [256, FC1_SIZE], name='fc1_w')
    fc14_b = tf.placeholder(tf.float32, [FC1_SIZE], name='fc1_b')

    fc15_w = tf.placeholder(tf.float32, [FC1_SIZE, FC2_SIZE], name='fc2_w')
    fc15_b = tf.placeholder(tf.float32, [FC2_SIZE], name='fc2_b')

    fc16_w = tf.placeholder(tf.float32, [FC2_SIZE, NUM_LABELS], name='fc3_w')
    fc16_b = tf.placeholder(tf.float32, [NUM_LABELS], name='fc3_b')

    parameters = (
        cv1_w, cv1_b, cv2_w, cv2_b, cv3_w, cv3_b,
        cv4_w, cv4_b, cv5_w, cv5_b, cv6_w, cv6_b,
        cv7_w, cv7_b, cv8_w, cv8_b, cv9_w, cv9_b,
        cv11_w, cv11_b, cv12_w, cv12_b, cv13_w, cv13_b,
        fc14_w, fc14_b, fc15_w, fc15_b, fc16_w, fc16_b)
