...
tf.app.flags.DEFINE_string('ip_address', '172.22.209.115', '''The ip address of parameter server''')
...
def main():
    ...
    logits, cost, update_op, lr = compute_cost(logits, target_a, target_b, lam_tensor, FLAGS.lr,
                                               trainable_vars=update_vars+bn_and_se_vars)
    ...
    replace = replace_trainable_vars(weights, parameters)   # recv initial parameters from server
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(1, 301):
                ...
                for worker_batch in worker_batches:
                    ...
                    bn, _, temp_cost, decay_lr, updated, updated_vars, train_acc = sess.run([tf.get_collection(tf.GraphKeys.UPDATE_OPS),
                                                                                         replace, cost, lr, update_op, update_vars, accuracy],
                                                                feed_dict={X: mix_x, target_a: y_a, target_b: y_b, lam_tensor: lam,
                                                                           parameters: p, is_train: True})
