...
tf.app.flags.DEFINE_integer('paras_length', 20735, """Number of gradients byte.""")
...
class BSP():
    ...
    def predict(self):
        x = useful_funcs.create_placeholder()
        parameters_tensor = useful_funcs.inference(x)
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, './save_model/model_9914/model.ckpt')
            while True:
                paras = 0
...
def main():
    ...
    server_socket = useful_funcs.tcp_connection(FLAGS.port, FLAGS.nums_worker)
    queue = useful_funcs.create_queue(FLAGS.nums_worker)
    sockets = []
    worker_id = 1
    while True:
        ...
        if len(sockets) == FLAGS.nums_worker:
            ...
            for socket in sockets:
                bsp_init = threading.Thread(target=useful_funcs.send_init_parameters, args=(socket, init_parameters, worker_id))
                bsp_init.start()
                bsp_init.join()
                worker_id += 1
            worker_id = 1
