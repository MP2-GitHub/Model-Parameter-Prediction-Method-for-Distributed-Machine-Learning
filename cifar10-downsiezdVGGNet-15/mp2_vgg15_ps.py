...
import pickle as pk
import threading
from mp2_vgg15_init import initialize_parameters
import mp2_vgg15_functions
import numpy as np

FLAGS = tf.app.flags.FLAGS
# Neural Network Configuration
tf.app.flags.DEFINE_integer('nums_worker', 8, """Number of workers.""")
tf.app.flags.DEFINE_integer('parameters_length', 2081441, """Number of model parameters byte.""")
...
class BSP():
    ...
    def predict(self):
        x = mp2_vgg15_functions.create_placeholder()
        parameters_tensor = mp2_vgg15_functions.inference(x)
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, './model_vgg16/model.ckpt')
            while True:
                paras = np.zeros([1, 260160])
                ...
                paras = paras.reshape([-1, 8, 8, 4065])
...
def main():
    ...
    server_socket = mp2_vgg15_functions.tcp_connection(FLAGS.port, FLAGS.nums_worker)
    queue = mp2_vgg15_functions.create_queue(FLAGS.nums_worker)
    sockets = []
    worker_id = 1
    while True:
        ...
        if len(sockets) == FLAGS.nums_worker:
            ...
            for socket in sockets:
                bsp_init = threading.Thread(target=mp2_vgg15_functions.send_init_parameters, args=(socket, init_parameters, worker_id))
                bsp_init.start()
                bsp_init.join()
                worker_id += 1
            worker_id = 1
