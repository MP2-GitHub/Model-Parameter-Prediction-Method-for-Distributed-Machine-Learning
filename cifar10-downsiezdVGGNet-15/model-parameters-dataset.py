...
def extra_layers():
    ...
    K = list(np.arange(start=150, stop=4650, step=150)) # 150，300， 450， 。。。。。
    D = np.zeros([1, 260160])
    t = 0
    for k in K:
        with open('./paras-dataset/vgg16_parameters_{}'.format(k), 'rb') as f:
            data = pk.load(f)

            for paras_1234 in data:
                t += 1
                train_xs = paras_1234[0, 0:260160]          # 3*3*3*64+64+3*3*64*64+64+3*3*64*128+128+3*3*128*128+128 = 260160
                D = np.r_[D, train_xs.reshape(1, 260160)]

                if t % 100 == 0:
                    print('----------------completed {} iterations combine!-----------------'.format(t))

                if t % 500 == 0:
                    with open('./paras-dataset/xs_layers_1234_{}'.format(t), 'wb') as f:
                        f.write(pk.dumps(D[1:], protocol=4))
                        print('已经保存数据集！')
                        print('数据集维度：{}'.format(D[1:].shape))
                    D = np.zeros([1, 260160])
