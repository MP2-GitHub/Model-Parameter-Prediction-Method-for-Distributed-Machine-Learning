...
import time
import seblock
from mp2_vgg15_init import create_placeholder
...
def main():
    ...
    obtain_updated_vars_op = trainable_vars[0: 8]
    ...
    with tf.Session() as sess:
        ...
        with open('./downsized_vgg16_results1/test_accs', 'wb') as f:
            f.write(pk.dumps(test_accs))
        with open('./downsized_vgg16_results1/loss', 'wb') as f:
            f.write(pk.dumps(costs))
        with open('./downsized_vgg16_results1/recall', 'wb') as f:
            f.write(pk.dumps(recalls))
        with open('./downsized_vgg16_results1/precisions', 'wb') as f:
            f.write(pk.dumps(precisions))
        with open('./downsized_vgg16_results1/F1_scores', 'wb') as f:
