...
def extra_layers():
    ...
    t = 0
    for k in K:
        with open('./model_para/Alexnet_parameters_{}'.format(k), 'rb') as f:
            data = pk.load(f)

            for paras_1234 in data:
                t += 1
                train_xs = paras_1234[0, 0:617344]  # 3*3*3*96+96+5*5*96*256+256+3*3*256*384+384
                D = np.r_[D, train_xs.reshape(1, 617344)]

                if t % 100 == 0:
                    print('----------------completed {} iterations combine!-----------------'.format(t))

                if t % 500 == 0:
                    with open('./model_para/xs_layers_1234_{}'.format(t), 'wb') as f:
                        f.write(pk.dumps(D[1:], protocol=4))
                        print('已经保存数据集！')
                        print('数据集维度：{}'.format(D[1:].shape))
                    D = np.zeros([1, 617344])
