...
class ResNet18(object):
    ...
    def build_model(self):
        ...
        x = utils._conv(self._images, 3, 16*self._hp.k, 1, name='init_conv')
        ...
        for i in range(1, 5):
                ...
                if i == 1:
                    ...
                    shortcut = x
                    # Residual
                    x = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='conv_11')
                    # x = utils._bn(x, self.is_train, name='bn_1')
                    x = utils._relu(x)
                    x = utils._conv(x, 3, filters[i - 1], 1, name='conv_12')
                    # x = utils._bn(x, self.is_train, name='bn_2')
                    # Merge
                    x = x + shortcut
                    x = utils._relu(x)
                    ...
                    for j in range(1, 3):
                        with tf.variable_scope('unit_%d_%d' % (i, j)) as scope:
                            ...
                            shortcut = x
                            # Residual
                            x = utils._conv(x, 3, filters[i - 1], 1, name='conv_1')
                            # x = utils._bn(x, self.is_train, name='bn_1')
                            x = utils._relu(x)
                            x = utils._conv(x, 3, filters[i - 1], 1, name='conv_2')
                            # x = utils._bn(x, self.is_train, name='bn_2')
                            # Merge
                            x = x + shortcut
                            x = utils._relu(x)
                ...
                if i == 2:
                    with tf.variable_scope('unit_%d_0' % i) as scope:
                        print('\tBuilding residual unit: %s' % scope.name)
                    shortcut = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='shortcut_c1')
                    # Residual
                    x = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='conv_21')
                    # x = utils._bn(x, self.is_train, name='bn_1')
                    x = utils._relu(x)
                    x = utils._conv(x, 3, filters[i - 1], 1, name='conv_22')
                    # x = utils._bn(x, self.is_train, name='bn_2')
                    # Merge
                    x = x + shortcut
                    x = utils._relu(x)
                ...
                if i == 3:
                    with tf.variable_scope('unit_%d_0' % i) as scope:
                        print('\tBuilding residual unit: %s' % scope.name)
                    shortcut = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='shortcut_c2')
                    # Residual
                    x = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='conv_31')
                    # x = utils._bn(x, self.is_train, name='bn_1')
                    x = utils._relu(x)
                    x = utils._conv(x, 3, filters[i - 1], 1, name='conv_32')
                    # x = utils._bn(x, self.is_train, name='bn_2')
                    # Merge
                    x = x + shortcut
                    x = utils._relu(x)
                ...
                if i == 4:
                    with tf.variable_scope('unit_%d_0' % i) as scope:
                        print('\tBuilding residual unit: %s' % scope.name)
                    shortcut = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='shortcut_c3')
                    # Residual
                    x = utils._conv(x, 3, filters[i - 1], strides[i - 1], name='conv_41')
                    # x = utils._bn(x, self.is_train, name='bn_1')
                    x = utils._relu(x)
                    x = utils._conv(x, 3, filters[i - 1], 1, name='conv_42')
                    # x = utils._bn(x, self.is_train, name='bn_2')
                    # Merge
                    x = x + shortcut
                    x = utils._relu(x)
    ...
    def build_train_op(self):
        ...
        self.lr = tf.train.exponential_decay(learning_rate=0.001, global_step=self.step, decay_steps=50,
                                             decay_rate=0.999, staircase=True)
        self.trainable_vars = tf.trainable_variables()
        self.update_vars_opt = self.trainable_vars[0: 32]
