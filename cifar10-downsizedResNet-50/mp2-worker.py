...
tf.app.flags.DEFINE_integer('len', 47093521, """Size of initial parameters byte.""")
...
tf.app.flags.DEFINE_string('ip_address', '10.1.2.134', '''The ip address of parameter server''')
...
def train(num_classes, num_residual_units, k):
    ...
    X, y_a, y_b, lam_tensor, parameters = initializer_50.create_placeholder()

    hp = ResNet_50.HParams(batch_size=FLAGS.mini_batch_size,
                        num_classes=num_classes,
                        num_residual_units=num_residual_units,
                        k=k,
                        initial_lr=FLAGS.lr)
    network = ResNet_50.ResNet18(hp=hp, images=X, labels_a=y_a, labels_b=y_b, lam=lam_tensor)
    network.build_model()
    ...
    train_xs, train_ys, test_feeds = loads_data(X, y_a)
