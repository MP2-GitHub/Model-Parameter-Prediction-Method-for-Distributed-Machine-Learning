...
def extra_layers():
    ...
    t = 0
    for k in K:
        with open('./model_parameters/vgg15_parameters_{}'.format(k), 'rb') as f:
            data = pk.load(f)

            for paras_1234567 in data:
                t += 1
                train_xs = paras_1234567[0, 0:351872]
                D = np.r_[D, train_xs.reshape(1, 351872)]

                if t % 100 == 0:
                    print('----------------completed {} iterations combine!-----------------'.format(t))

                if t % 500 == 0:
                    with open('./dataset-cifar100-vgg15/xs_layers_12345_{}'.format(t), 'wb') as f:
                        f.write(pk.dumps(D[1:], protocol=4))
                        print('已经保存数据集！')
                        print('数据集维度：{}'.format(D[1:].shape))
                    D = np.zeros([1, 351872])
