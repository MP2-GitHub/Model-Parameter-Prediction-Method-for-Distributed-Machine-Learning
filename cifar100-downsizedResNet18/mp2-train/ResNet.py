...
class ResNet18(object):
        ...
        for i in range(1, 5):
            ...
            with tf.variable_scope('unit_%d_0' % i) as scope:
                ...
                if i == 1:
                    ...
                    # shortcut = utils._bn(shortcut, self.is_train, name='shortcut_bn1')
                    # shortcut = utils._conv(shortcut, 3, filters[i-1], strides[i-1], name='shortcut_c2')
                    # shortcut = utils._bn(shortcut, self.is_train, name='shortcut_bn2')
                # Residual
                x = utils._conv(x, 3, filters[i-1], strides[i-1], name='conv_1')
    ...
    def build_train_op(self):
        ...
        self.lr = tf.train.exponential_decay(learning_rate=0.001, global_step=self.step, decay_steps=50, decay_rate=0.999, staircase=True)
        self.trainable_vars = tf.trainable_variables()
        self.update_vars_opt = self.trainable_vars[0: 20]
