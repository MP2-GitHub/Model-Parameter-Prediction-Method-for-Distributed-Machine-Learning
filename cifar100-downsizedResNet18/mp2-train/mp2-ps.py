...
tf.app.flags.DEFINE_integer('parameters_length', 1485473, """Number of model parameters byte.""")
...
class BSP():
    ...
    def predict(self):
        x = functions.create_placeholder()
        parameters_tensor = functions.inference(x)
        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, './save-model/model.ckpt')
            while True:
                paras = np.zeros([1, 185664])
                ...
                paras = paras.reshape([-1, 8, 8, 2901])
...
def main():
    ...
    server_socket = functions.tcp_connection(FLAGS.port, FLAGS.nums_worker)
    queue = functions.create_queue(FLAGS.nums_worker)
    sockets = []
    worker_id = 1
    while True:
        ...
        if len(sockets) == FLAGS.nums_worker:
            ...
            for socket in sockets:
                bsp_init = threading.Thread(target=functions.send_init_parameters, args=(socket, init_parameters, worker_id))
                bsp_init.start()
                bsp_init.join()
                worker_id += 1
            worker_id = 1
