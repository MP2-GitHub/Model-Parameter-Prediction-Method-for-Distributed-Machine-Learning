...
def forward_propagation(input_tensor, input_images, seed):
    ...
    with tf.variable_scope('Decoder-2'):
        w21 = tf.get_variable('fcn1-weight', [flatten.shape[1], 2570], initializer=tf.truncated_normal_initializer(stddev=0.1))
        z21 = tf.matmul(flatten, w21)
        y21_hat = tf.reshape(z21, [-1, 2570])
    ...
    decoder_fcn_w21 = y21_hat[seed, 0: 2560]
    decoder_fcn_w21 = tf.reshape(decoder_fcn_w21, [256, 10])
    decoder_fcn_b21 = y21_hat[seed, 2560: 2570]
...
def main(epoch_nums):
    ...
    trainable_vars = variables[0:6] + variables[18:]  # 只训练输入和输出层模型参数
    untrained_vars = variables[6: 18]                 # 中间结构模型参数用之前已训练好的模型参数
    print('trainable_variables: ')
    print(trainable_vars)
    print('untrainaed_variables: ')
    print(untrained_vars)

    logits, loss, optimizer = compute_cost(logits, eval_ya, eval_yb, lam_tensor, 0.001, trainable_vars)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1, epoch_nums+1):
            ...
            for xs in input_paras:
                ...
                if test_acc > max_acc:
                    max_acc = test_acc
                    saver2.save(sess, "./model2-vgg11/save_models/model.ckpt")    # 保存精度最高时候的模型
                if iterations % 1 == 0:
    ...
    with open('./model2-vgg11/results/exp1/cost', 'wb') as f:
        f.write(pk.dumps(costs))
    with open('./model2-vgg11/results/exp1/test_acc', 'wb') as f:
